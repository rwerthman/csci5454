...
\begin{enumerate}
  ...
  number of customers helped in a hardward store with limited supplies.}
    \begin{enumerate}
      ...
      A = \{y,x\}, B = \{y\}, C=\{y\}.
      In this case OfflineCustomerHelp would at worst sell two items -- item y to
      customer A and then item x either to customer B or C.  The
      OnlineCustomerHelp would at worst sell item y because it saw customer
      A first and chose the first item that had not been sold yet.
      OnlineHelpCustomer ends up selling one item which is at least half of
      the items sold in OfflineCustomerHelp's solution.\\
      \\
      We can generalize this and say that the cost for the optimal offline
      algorithm OfflineCustomerHelp gives optimal solution $n$ which means
      if there are $n$ customers and $n$ items all customers are matched with
      at least one item.
      The cost of the solution for the online algorithm OnlineCustomerHelp is at
      worst $n/2$ which means if there are $n$ items and $n$ customers at least
      half of the customers will be matched with an item.
      This gives a competitive ratio of OPT/ALG = n/(n/2) = 2.
      ...
      randomized version of the online selling algorithm is $\le 2$.}
    \end{enumerate}
