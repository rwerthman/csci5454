...
\begin{enumerate}
  ...
  number of customers helped in a hardward store with limited supplies.}
    \begin{enumerate}
      ...
      number of items in a customer's set.  This makes out equation above
      $$
      \frac{OPT}{ALG} = \frac{n-mean(|S_n|)}{n-E[X]} < 2
      $$
      Because you can say the $E[X]$ is the average if we have $n$ customers
      than we can expect $n/2$ customers to receive an item.  This simplifies
      our competitive ration equation to 
      $$
      \frac{n-mean(|S_n|)}{n-E[X]} = \frac{n-mean(|S_n|)}{n-n/2} =
      \frac{n-mean(|S_n|)}{n/2} < 2 
      $$
      Furthermore the averge number of items in the customers' sets will $\ge 1$
      and $\le n$.  This gives us two boundaries to test for the competitive
      ration.  Setting $mean(|S_n|) = n$ the competive ratio holds.
      $$
      \frac{n-n}{n/2} = 0 < 2
      $$
